...
for (i in 1:NROW(detail.info)){
  ...
  tryCatch({page_content <- fromJSON(url)
           ...
           detail.info$price_note[i] <- ifelse((!is.null(items[which(items$name %in% c("Celkov? cena", "Cena")),]) 
                                                && nrow(items[which(items$name %in% c("Celkov? cena", "Cena")),]) > 0),
                                               paste0(unlist(items[which(items$name %in% c("Celkov? cena", "Cena")),]$notes), collapse = ", "), NA)
           
           detail.info$price_note2[i] <- ifelse((!is.null(items[which(items$name == "Pozn?mka k cen?"),]) 
                                                 && nrow(items[which(items$name == "Pozn?mka k cen?"),]) > 0),
                                                paste0(unlist(items[which(items$name == "Pozn?mka k cen?"),]$value), collapse = ", "), NA)
           
           detail.info$currency[i] <- ifelse((!is.null(items[which(items$name %in% c("Celkov? cena", "Cena")),]) 
                                              && nrow(items[which(items$name %in% c("Celkov? cena", "Cena")),]) > 0),
                                             paste0(unlist(items[which(items$name %in% c("Celkov? cena", "Cena")),]$currency), collapse = ", "), NA)
           
           detail.info$id[i] <- ifelse((!is.null(items[which(items$name %in% c("ID zak?zky", "ID")),]) 
                                        && nrow(items[which(items$name %in% c("ID zak?zky", "ID")),]) > 0),
                                       unlist(items[which(items$name %in% c("ID zak?zky", "ID")),]$value), NA)
           ...
           detail.info$aktualizace[i] <- ifelse((!is.null(items[which(items$name == "Aktualizace"),]) 
                                                ...
                                                ifelse(unlist(items[which(items$name == "Aktualizace"),]$value) == "Dnes",
                                                       ...
                                                       ifelse(unlist(items[which(items$name == "Aktualizace"),]$value) == "V?era",
           ...
           detail.info$vlastnictvi[i] <- ifelse((!is.null(items[which(items$name == "Vlastnictv?"),]) 
                                                  && nrow(items[which(items$name == "Vlastnictv?"),]) > 0),
                                                 paste0(items[which(items$name == "Vlastnictv?"),]$value, collapse = ", "), NA)
           ...
           detail.info$podlazi[i] <- ifelse((!is.null(items[which(items$name == "Podla??"),]) 
                                              && nrow(items[which(items$name == "Podla??"),]) > 0),
                                             paste0(items[which(items$name == "Podla??"),]$value, collapse = ", "), NA)
           
           detail.info$zastavena_plocha[i] <- ifelse((!is.null(items[which(items$name == "Plocha zastav?n?"),]) 
                                                   && nrow(items[which(items$name == "Plocha zastav?n?"),]) > 0),
                                                  paste0(items[which(items$name == "Plocha zastav?n?"),]$value, collapse = ", "), NA)
           
           detail.info$uzitna_plocha[i] <- ifelse((!is.null(items[which(items$name == "U?itn? plocha"),]) 
                                              && nrow(items[which(items$name == "U?itn? plocha"),]) > 0),
                                             paste0(items[which(items$name == "U?itn? plocha"),]$value, collapse = ", "), NA)
           
           detail.info$podlahova_plocha[i] <- ifelse((!is.null(items[which(items$name == "Plocha podlahov?"),]) 
                                                      && nrow(items[which(items$name == "Plocha podlahov?"),]) > 0),
                                                     paste0(items[which(items$name == "Plocha podlahov?"),]$value, collapse = ", "), NA)
           ...
           detail.info$parkovani[i] <- ifelse((!is.null(items[which(items$name == "Parkov?n?"),]) 
                                           && nrow(items[which(items$name == "Parkov?n?"),]) > 0),
                                          paste0(items[which(items$name == "Parkov?n?"),]$value, collapse = ", "), NA)
           
           detail.info$garaz[i] <- ifelse((!is.null(items[which(items$name == "Gar??"),]) 
                                               && nrow(items[which(items$name == "Gar??"),]) > 0),
                                              paste0(items[which(items$name == "Gar??"),]$value, collapse = ", "), NA)
           ...
           detail.info$balkon[i] <- ifelse((!is.null(items[which(items$name == "Balk?n"),]) 
                                           && nrow(items[which(items$name == "Balk?n"),]) > 0),
                                          paste0(items[which(items$name == "Balk?n"),]$value, collapse = ", "), NA)
           
           detail.info$vytah[i] <- ifelse((!is.null(items[which(items$name == "V?tah"),]) 
                                           && nrow(items[which(items$name == "V?tah"),]) > 0),
                                          paste0(items[which(items$name == "V?tah"),]$value, collapse = ", "), NA)
           
           detail.info$energeticka_narocnost[i] <- ifelse((!is.null(items[which(items$name == "Energetick? n?ro?nost budovy"),]) 
                                              && nrow(items[which(items$name == "Energetick? n?ro?nost budovy"),]) > 0),
                                             paste0(items[which(items$name == "Energetick? n?ro?nost budovy"),]$value, collapse = ", "), NA)
           ...
           detail.info$bezbarierovy[i] <- ifelse((!is.null(items[which(items$name == "Bezbari?rov?"),]) 
                                              && nrow(items[which(items$name == "Bezbari?rov?"),]) > 0),
                                             paste0(items[which(items$name == "Bezbari?rov?"),]$value, collapse = ", "), NA)
           
           detail.info$vybaveni[i] <- ifelse((!is.null(items[which(items$name == "Vybaven?"),]) 
                                              && nrow(items[which(items$name == "Vybaven?"),]) > 0),
                                             paste0(items[which(items$name == "Vybaven?"),]$value, collapse = ", "), NA)
           
           detail.info$umisteni[i] <- ifelse((!is.null(items[which(items$name == "Um?st?n? objektu"),]) 
                                              && nrow(items[which(items$name == "Um?st?n? objektu"),]) > 0),
                                             paste0(items[which(items$name == "Um?st?n? objektu"),]$value, collapse = ", "), NA)
           ...
           detail.info$elektro[i] <- ifelse((!is.null(items[which(items$name == "Elekt?ina"),]) 
                                             && nrow(items[which(items$name == "Elekt?ina"),]) > 0),
                                            paste0(as.data.frame(items[which(items$name == "Elekt?ina"),]$value)[,2], collapse = ", "), NA)
           ...
           detail.info$topeni[i] <- ifelse((!is.null(items[which(items$name == "Topen?"),]) 
                                          && nrow(items[which(items$name == "Topen?"),]) > 0),
                                         paste0(as.data.frame(items[which(items$name == "Topen?"),]$value)[,2], collapse = ", "), NA)
           ...
           detail.info$nastehovani[i] <- ifelse((!is.null(items[which(items$name == "Datum nast?hov?n?"),]) 
                                                 && nrow(items[which(items$name == "Datum nast?hov?n?"),]) > 0),
                                                unlist(items[which(items$name == "Datum nast?hov?n?"),]$value), NA)
...
#Formula
formula <- as.formula('Cena~ Uzitna_plocha +Stav_objektu + Obec + Podlazi + Zahrada + Podkategorie' )


#1 LM 
lm <- lm(formula, df_train)
predict(lm, df_test)
...
library(rpart)
library(rpart.plot)
rpart <- rpart(formula, df_train)
rpart.plot(rpart)

df_test$predicted_CENA <- predict(rpart, df_test)
...
df_my <- data.frame(
  Cena <- 4730000,
  Uzitna_plocha <- 58,
  Stav_objektu <- as.factor('Velmi dobr?'),
  Obec <- as.factor('Praha 9 - ??st obce Vyso?any'), 
  Podlazi <- 2,
  Zahrada <- 0,
